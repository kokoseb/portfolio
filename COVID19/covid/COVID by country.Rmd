...
---
In this project, we graph cumulative deaths, cumulative cases, and daily cases from COVID-19 in selected countries using the coronavirus package.
...
countries <- c(
  ...
  #"India"
  #"Argentina"
  #note: remove the hashtag to select the country, add a hastag to unselect. The full list of countries available for analysis are listed in the Appendix
)
```
...
The following visualisation shows the number of cumulative deaths vs. days elapsed since the 10th confirmed death in each country.
...
Similar operations as described above were used to obtain a visualization for cumulative cases for the selected countries
```{r}
#Filter the data
df1= country_data <- coronavirus %>%
...
ggplot(data = country_data,
  ...
  labs(
    ...
    title = "Cumulative confirmed cases from COVID-19, selected countries",
    subtitle = glue("Data as of", as_of_date_formatted, .sep = " "),
    caption = "Source: github.com/RamiKrispin/coronavirus"
  )
...
The following shows the daily number of new cases vs days since the tenth confirmed death for selected countries
```{r}
ggplot(data = country_data,
       mapping = aes(x = days_elapsed, 
                     y = tot_cases, 
