...
library(ggrepel)
library(DT)
#code from code preview, reads file to load data
library(readr)
...
covid_data <- read_excel("U:/DS241/cdc covid death data.xlsx")
```
...
The following visualisation shows the number of deaths vs. the date in each state. The as-of date will be the day before the Excel spreadsheet was exported from the CDC website.
